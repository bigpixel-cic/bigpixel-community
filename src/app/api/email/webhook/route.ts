    ...
    switch (event.type) {
      case 'email.received': {
        ...
        if (emailError) {
          ...
          await resend.emails.send({
            from: 'System <system@bigpixel.org.uk>',
            to: ['team@bigpixel.org.uk'],
      ...
      case 'email.complained':
        ...
        await resend.emails.send({
          from: 'System <system@bigpixel.org.uk>',
          to: ['team@bigpixel.org.uk'],
          subject: `Complaint: ${event.data.subject}`,
          html: `<p>User marked email as spam:</p><ul><li>From: ${event.data.from}</li><li>To: ${event.data.to.join(', ')}</li><li>Subject: ${event.data.subject}</li></ul>`,
        });
